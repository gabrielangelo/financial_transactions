defmodule FinancialTransactions.AccountsTest do
  ...
  describe "accounts" do
    ...
    setup do
      ...
      {:ok, user: user, valid_account_attrs: valid_account_attrs, invalid_account_attrs: invalid_account_attrs}
    end
    ...
    test "get_account!/1 returns the account with given id", %{valid_account_attrs: valid_account_attrs} do
      account = account_fixture(valid_account_attrs)
      assert Accounts.get_account!(account.id).id == account.id
    end

    test "create_account/1 with valid data creates a account", %{valid_account_attrs: valid_account_attrs} do
      account = account_fixture(valid_account_attrs)
    ...
    test "create_account/1 with invalid data returns error changeset", %{invalid_account_attrs: invalid_account_attrs} do
      assert {:error, %Ecto.Changeset{} = changeset} = Accounts.create_account(invalid_account_attrs)
      assert changeset.errors == [company_id: {"Account must be vinculated at least one user or company", []}]
    end

    test "update_account/2 with valid data updates the account", %{valid_account_attrs: valid_account_attrs} do
      account = account_fixture(valid_account_attrs)
      update_attrs = %{
        name: "updated name",
